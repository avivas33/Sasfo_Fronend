...
export function P2PTable({ onP2PSelect, selectedP2PId, filterByEstado }: P2PTableProps) {
  ...
  // Fetch P2P con React Query
  const { data, isLoading, isError, error } = useP2PList({
    search: debouncedSearch || undefined,
    page,
    pageSize,
    tipoP2P: 0, // P2P normal (no Postes)
  ...
  const handleNewP2P = () => {
    navigate("/viabilidades/p2p/nuevo");
  };

  const handleEdit = (p2p: P2P, e: React.MouseEvent) => {
    e.stopPropagation();
    navigate(`/viabilidades/p2p/nuevo?id=${p2p.ID_P2P}`);
  };
  ...
  const handleDelete = async (p2p: P2P, e: React.MouseEvent) => {
    ...
    if (p2p.Estado === EstadoP2P.Cancelado) {
      toast.error("Esta orden P2P ya fue cancelada");
      return;
    }

    if (p2p.Estado === EstadoP2P.Completado) {
      toast.error("No se puede cancelar una orden P2P completada");
      return;
    }

    if (confirm(`¿Está seguro de cancelar la orden P2P #${p2p.ID_P2P}?`)) {
      try {
        await cancelarP2P.mutateAsync(p2p.ID_P2P);
        toast.success("Orden P2P cancelada exitosamente");
      } catch (error) {
        toast.error("Error al cancelar orden P2P");
      }
    }
  };
  ...
  const handleAprobar = async (p2p: P2P, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.success("Orden P2P aprobada exitosamente");
    } catch (error) {
      toast.error("Error al aprobar orden P2P");
    }
  };
  ...
  const handleCompletar = async (p2p: P2P, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.success("Orden P2P completada exitosamente");
    } catch (error) {
      toast.error("Error al completar orden P2P");
    }
  };
  ...
  return (
    <Box className="flex-1 flex flex-col bg-white">
      ...
      <Box className="border-b p-4" style={{ borderColor: "var(--gray-6)" }}>
        <Flex align="center" justify="between">
          <Flex align="center" gap="4">
            <Text size="4" weight="medium">Listado de P2P</Text>
          ...
          <Flex gap="2">
            ...
            <Button size="2" onClick={handleNewP2P}>
      ...
      <Box className="p-4 border-b" style={{ borderColor: "var(--gray-6)" }}>
        <TextField.Root
          placeholder="Buscar por nombre de punto..."
          size="2"
          value={searchTerm}
          ...
          <Table.Root variant="surface">
            <Table.Header>
              <Table.Row style={{ backgroundColor: "var(--gray-3)" }}>
                ...
                <Table.ColumnHeaderCell>CREACION</Table.ColumnHeaderCell>
              ...
              {isLoading ? (
                ...
                <Table.Row>
                  <Table.Cell colSpan={10}>
                    <Box className="p-8 text-center">
                      <Text size="2" style={{ color: "var(--gray-11)" }}>
                        No se encontraron órdenes P2P que coincidan con tu búsqueda.
                      </Text>
                    </Box>
                  </Table.Cell>
                </Table.Row>
