...
import { ChevronLeft, ChevronRight, Check, Plus, ArrowLeft, FileText, Loader2 } from "lucide-react";
...
interface NuevoP2PWizardProps {
  p2pId?: number;
}

export function NuevoP2PWizard({ p2pId }: NuevoP2PWizardProps) {
  ...
  // idProcesoViabilidad: 1=Por Aprobar, 2=En Proceso, 3=Completada, 4=Cancelada/No Aprobada
  ...
  const handleNuevaViabilidad = (punto: "Punto1" | "Punto2") => {
    // Navegar a crear nueva viabilidad con parámetros
    navigate(`/viabilidades/nueva-solicitud?parametro1=${punto}&parametro2=${p2pData.ID_P2P || 0}`);
  };
  ...
  const handleSelectViabilidad = (viabilidad: Viabilidad, punto: "Punto1" | "Punto2") => {
    if (punto === "Punto1") {
      ...
      toast.success("Punto 1 asignado correctamente");
    } else {
      setP2pData(prev => ({
        ...prev,
      ...
      toast.success("Punto 2 asignado correctamente");
    }
  };

  const handleAprobar = async () => {
    if (!p2pData.ID_P2P) {
      toast.error("No hay P2P para aprobar");
      return;
    }

    try {
      await aprobarP2P.mutateAsync(p2pData.ID_P2P);
      toast.success("P2P aprobado exitosamente");
      navigate("/viabilidades/p2p");
    } catch (error) {
      toast.error("Error al aprobar P2P");
    }
  };
  ...
  const handleSave = async () => {
    try {
      if (!p2pData.ID_P2P) {
        ...
        toast.success("P2P creado exitosamente");
      }
    } catch (error) {
      toast.error("Error al guardar P2P");
    }
  };
  ...
  return (
    <Box className="flex-1 flex flex-col bg-white">
      ...
      <Box className="border-b p-4" style={{ borderColor: "var(--gray-6)" }}>
        <Flex direction="column" gap="3">
          ...
          <Flex align="center" gap="2">
            ...
            <Text size="2" style={{ color: "var(--gray-11)" }}>P2P</Text>
            <Text size="2" style={{ color: "var(--gray-11)" }}>/</Text>
      ...
      <Box className="border-b p-4" style={{ borderColor: "var(--gray-6)", backgroundColor: "var(--gray-2)" }}>
        <Flex align="center" gap="3">
          <Button variant="soft" color="gray" onClick={() => navigate("/viabilidades/p2p")}>
          ...
          <Button
            variant="outline"
            color="amber"
            onClick={() => handleNuevaViabilidad("Punto2")}
      ...
      <Box className="flex-1 overflow-auto p-4">
          ...
          <Box>
            <Card className="mb-4">
              <Flex direction="column" gap="3" className="p-4">
                <Text size="3" weight="bold">Solicitud de Viabilidad</Text>
                <Text size="2" style={{ color: "var(--gray-11)" }}>
                  Seleccione o cree las viabilidades para los puntos del P2P
                </Text>
              </Flex>
            </Card>
          ...
          <Box>
            <Card className="mb-4">
              <Flex direction="column" gap="3" className="p-4">
                ...
                <Text size="2" style={{ color: "var(--gray-11)" }}>
                  Viabilidades completadas con órdenes de servicio disponibles
                </Text>
              </Flex>
            </Card>
        ...
        {currentStep === 2 && (
          <Box>
            <Card className="mb-4">
              <Flex direction="column" gap="3" className="p-4">
                <Text size="3" weight="bold">Resumen del P2P</Text>
                <Text size="2" style={{ color: "var(--gray-11)" }}>
                  Revise la información y apruebe el P2P
                </Text>
              </Flex>
            </Card>
            ...
            <Table.Root variant="surface">
              <Table.Header>
                <Table.Row>
                  ...
                  <Table.ColumnHeaderCell>Nombre Pto1</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>Punto 1</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>Nombre Pto2</Table.ColumnHeaderCell>
